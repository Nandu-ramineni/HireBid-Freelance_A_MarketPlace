...
export default function CompletedBid({ completedBid }) {
    if (!completedBid) return null
            ...
            <div className="relative flex items-start gap-4">
                ...
                <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-2">
                        ...
                        <span className="text-sm text-emerald-600 font-medium">{formatDate(completedBid?.acceptedAt)}</span>
                    </div>
                    ...
                    <div className="flex items-center gap-3 p-4 bg-white/60 backdrop-blur-sm rounded-lg border border-emerald-100">
                        <Avatar className="w-10 h-10 border-2 border-emerald-200">
                            <AvatarImage src={completedBid?.freelancer?.profile || "/placeholder.svg"} />
                            <AvatarFallback className="bg-emerald-100 text-emerald-700 font-semibold">
                                {getInitials(completedBid?.freelancer?.firstName, completedBid?.freelancer?.lastName)}
                            </AvatarFallback>
                        </Avatar>
                        ...
                        <div className="flex-1">
                            <div className="flex items-center gap-2">
                                <span className="font-semibold text-gray-900">
                                    {completedBid?.freelancer?.firstName} {completedBid?.freelancer?.lastName}
                                </span>
                                {completedBid?.freelancer?.rating && (
                                    <div className="flex items-center gap-1">
                                        <div className="flex">
                                            {[...Array(5)].map((_, i) => {
                                                const isActive = i < Math.floor(completedBid?.freelancer?.rating || 0);
                                                return (
                                                    <div
                                                        key={i}
                                        ...
                                        <span className="text-xs text-gray-600 ml-1">{completedBid?.freelancer?.rating}</span>
                                    </div>
                                )}
                            </div>
                        ...
                        {completedBid?.bid?.amount && (
                            <div className="text-right">
                                <div className="text-lg font-bold text-emerald-700">₹{completedBid?.bid?.amount.toLocaleString()}</div>
                    ...
                    <div className="mt-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
                        ...
                        <p className="text-sm text-blue-700 mt-1">
                            Your project has been successfully completed. You can now leave feedback for the freelancer and close the project.
                        </p>
                        <div className="flex justify-end">
                            <Button variant="outline" className="mr-2" onClick={() => navigate(`/jobs/${completedBid?.bid?.jobId}/feedback`)}>
                                Close Project
                            </Button>
                            <Button variant="default" className="bg-emerald-600 text-white hover:bg-emerald-700" onClick={() => navigate(`/client-dashboard/reviews`)}>
                                Leave Feedback
                            </Button>
                        </div>
                    </div>
